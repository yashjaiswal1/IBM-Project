...
def addcollege():
    ...
    if request.method == 'POST':
        ...
        doc = {'college_name': college_name, 'college_category': college_category,
               'ur_spoc': ur_spoc, 'city': city, 'region': region, 'type': 'admin_college_add'}
        db.save(doc)
...
class EventForm(Form):
    events = SelectField('Events', choices=[("Technical Fest", "Technical Fest"), ("Conference", "Conference"),  (
        "Tech Session", "Tech Session"), ("Tech Workshop", "Tech Workshop"), ("Hackathon", "Hackathon"), ("SUR", "SUR")])
    event_name = StringField('Event Name', [validators.Length(
        min=1, max=200), validators.DataRequired()])
    tech_session_name = StringField(
        'Tech Session / Workshop Name', [validators.Length(min=1, max=200), validators.DataRequired()])
    ibm_sme_name = StringField('IBM SME Name', [validators.Length(
        min=1, max=200), validators.DataRequired()])
    bu = SelectField(
        'BU',
        choices=[('ISL', 'ISL'), ('IRL', 'IRL'), ('GBS', 'GBS'), ('GTS', 'GTS')])
    box_location_atten = StringField('Box location for Attendee list', [
                                     validators.Length(min=1, max=500), validators.DataRequired()])
    box_location_feed = StringField('Box location for Feedback', [
                                    validators.Length(min=1, max=500), validators.DataRequired()])
    box_location_winning_profile = StringField('Box location for Winning team profiles', [
                                               validators.Length(min=1, max=500), validators.DataRequired()])
    sponsorship_amount = StringField('Sponsorship Amount', [validators.Length(
        min=5, max=500), validators.DataRequired()], render_kw={"placeholder": "INR"})

    hackathon_name = StringField('Hackathon Name', [validators.Length(
        min=1, max=200), validators.DataRequired()])

    # ----------tech workshop---------
    institution = StringField(
        'Institution', [validators.Length(min=1), validators.DataRequired()])
    tech_area = StringField(
        'Technology', [validators.Length(min=1), validators.DataRequired()])
    no_of_sessions = StringField('Number of sessions', [
    ...
    event_posted = RadioField('Event posted on portal', [
                              validators.DataRequired()], choices=[('Yes', 'Yes'), ('No', 'No')])
    event_socialising = RadioField('Post event socialising?', [
                                   validators.DataRequired()], choices=[('Yes', 'Yes'), ('No', 'No')])
    ...
    # ----------Hackathon---------
    # First Prize Winners
    team_name1 = StringField(
        'Team Name', [validators.Length(min=1), validators.DataRequired()])
    team_member_name1 = StringField(
        'Member Name', [validators.Length(min=1), validators.DataRequired()])
    team_member_college1 = StringField(
        'College', [validators.Length(min=1), validators.DataRequired()])
    ...
    college_name = StringField(
        'College Name', [validators.Length(min=3), validators.DataRequired()])
    college_category = SelectField('College Category', [validators.DataRequired(
    )], choices=[("Platinum", "Platinum"), ("Gold", "Gold"),  ("Silver", "Silver")])
    ur_spoc = StringField('UR SPOC', [validators.Length(
        min=1, max=200), validators.DataRequired()])
    city = SelectField('City', [validators.DataRequired()], choices=[
                       ("Bangalore", "Bangalore"), ("C2", "C2"), ("Chennai", "Chennai")])
    region = StringField('Region', [validators.Length(
        min=1, max=200), validators.DataRequired()])
    no_of_participants = IntegerField(
        'No of participants', [validators.NumberRange(min=1)])
    no_of_registrations = IntegerField(
        'No of registrations', [validators.NumberRange(min=1)])
    no_of_abstracts = IntegerField('No of abstracts shortlisted', [
                                   validators.NumberRange(min=1)])
    no_of_finalist = IntegerField('No of Finalist Teams',
                                  [validators.NumberRange(min=1)])
    finaledate = DateField('Finale Date', [validators.DataRequired(
    )], default=datetime.date.today, format='%Y-%m-%d')
    speaker_name = StringField('Speakers', [validators.Length(
        min=1, max=200), validators.DataRequired()])
    jury_name = StringField('Jury', [validators.Length(
        min=1, max=200), validators.DataRequired()])
    additional_jury_name = StringField(
        'Additional Jury', [validators.Length(min=1, max=200)])
    winning_team_details = StringField('Winning Team Details', [
    ...
    project_startdate = DateField('Project Start Date', [
                                  validators.DataRequired()], default=datetime.date.today, format='%Y-%m-%d')
    project_enddate = DateField('Project End Date', [
                                validators.DataRequired()], default=datetime.date.today, format='%Y-%m-%d')
    fromdate = DateField(
        'From Date', default=datetime.date.today, format='%Y-%m-%d')
    todate = DateField(
        'To Date', default=datetime.date.today, format='%Y-%m-%d')
    status = RadioField('Status', [validators.DataRequired()], choices=[('Planned', 'Planned'), (
        'In Progress', 'In Progress'), ('Completed', 'Completed'), ('Cancelled', 'Cancelled')], default='Planned')
    portal_status = RadioField('Event Posted on Portal?', [validators.DataRequired(
    )], choices=[('Yes', 'Yes'), ('No', 'No')], default='Yes')
    post_event_social = RadioField('Post Event Socialising?', [
                                   validators.DataRequired()], choices=[('Yes', 'Yes'), ('No', 'No')], default='Yes')
    list_of_events = SelectField('List of Events', choices=[])
    theme = SelectField('Theme', [validators.DataRequired()], choices=[('Agile / DevOps', 'Agile / DevOps'), ('AI / ML', 'AI / ML'), ('Analytics', 'Analytics'), ('Blockchain', 'Blockchain'), ('Cloud', 'Cloud'), ('Databases', 'Databases'), ('Design Thinking', 'Design Thinking'), ('Green Data Center', 'Green Data Center'), ('IoT', 'IoT'), ('Leadership Talk', 'Leadership Talk'), ('Mainframe',
                                                                                                                                                                                                                                                                                                                                                                                            'Mainframe'), ('Networking', 'Networking'), ('Patents & IP', 'Patents & IP'), ('Programming Languages', 'Programming Languages'), ('Project Management', 'Project Management'), ('Quantum Computing', 'Quantum Computing'), ('Security', 'Security'), ('Soft Skills', 'Soft Skills'), ('Smarter Cities', 'Smarter Cities'), ('SW Engg.', 'SW Engg.'), ('Watson', 'Watson'), ('Others', 'Others')])
    topic = StringField(
        'Topic', [validators.Length(min=3), validators.DataRequired()])
    name_of_participants = StringField('Name of Participants. Separate using ;', [
                                       validators.Length(min=3), validators.DataRequired()])

    # ---------SUR---------
    sur_topic_name = StringField(
        'SUR Topic Name', [validators.Length(min=3), validators.DataRequired()])
    prof_name = StringField('Professor Name', [validators.Length(
        min=3), validators.DataRequired()], render_kw={"placeholder": "Name"})
    prof_college_name = StringField('College', [validators.Length(
        min=3), validators.DataRequired()], render_kw={"placeholder": "College"})
    technology = StringField(
        'Technology', [validators.Length(min=3), validators.DataRequired()])
    sur_grant_req = StringField('SUR Grant Request', [validators.Length(
        min=3), validators.DataRequired()], render_kw={"placeholder": "INR"})
    proposal_receipt_date = DateField(
        'Proposal Receipt date', default=datetime.date.today, format='%Y-%m-%d')
    proposal_submission_date = DateField(
        'Proposal Submission date', default=datetime.date.today, format='%Y-%m-%d')
    proposal_status = RadioField('Proposal Status', [validators.DataRequired()], choices=[('Approved', 'Approved'), ('Rejected', 'Rejected')],
                                 default='Approved')
    invoice_receipt_date = DateField(
        'Invoice receipt date', default=datetime.date.today, format='%Y-%m-%d')
    invoice_payout_date = DateField(
        'Invoice payout date', default=datetime.date.today, format='%Y-%m-%d')
    paper_publications = StringField(
        'Paper publications', [validators.Length(min=3)])
    type_of_event = StringField('Event type')
    conference_showcase = RadioField('Conference Showcase', [
                                     validators.DataRequired()], choices=[("Yes", "Yes"), ("No", "No")], default="Yes")
    conference_url = StringField('', [validators.Length(min=3)], render_kw={
                                 "placeholder": "Conference URL"})
    sur_proposal_location = StringField(
        'SUR City ', [validators.Length(min=3), validators.DataRequired()])
    project_url = StringField(
        'Project URL', [validators.Length(min=3), validators.DataRequired()])
   # -------SUR END------
    ...
    requestor = StringField(
        'Requestor Name', [validators.Length(min=3), validators.DataRequired()])
    destination = StringField(
        'Destination/s', [validators.Length(min=3), validators.DataRequired()])
    amount_k = StringField('Amount (US $ K)',
                           [validators.Length(min=5, max=500), validators.DataRequired()], default='$  ')
    travel_date = DateField(
        'Travel date', default=datetime.date.today, format='%Y-%m-%d')
    amount_m = StringField('Amount (US $ M)',
                           [validators.Length(min=5, max=500), validators.DataRequired()], default='$  ')
    po_date = DateField(
        'PO date', default=datetime.date.today, format='%Y-%m-%d')
    submited_finance_date = DateField(
        'Date Submitted to Finance', default=datetime.date.today, format='%Y-%m-%d')
    ...
    if request.method == 'POST':
        ...
        if form.validate_on_submit_hackathon() == True:
            ...
            if ((finaledate > enddate) or (finaledate < startdate)):
                flash(
                    'Hackathon Finale Date should lie within the Start Date and End Date', 'danger')
            ...
            list_team1 = [x for x in zip(
                team_member_name1, team_member_college1)]
            ...
            list_team2 = [x for x in zip(
                team_member_name2, team_member_college2)]
            ...
            list_team3 = [x for x in zip(
                team_member_name3, team_member_college3)]
...
def edit_event(id):
    ...
    if db[id]['type'] == 'SUR':
        ...
        form.proposal_receipt_date.data = datetime.datetime.strptime(
            db[id]['proposal_receipt_date'].replace('-', ''), "%Y%m%d")
        ...
        form.project_startdate.data = datetime.datetime.strptime(db[id]['project_startdate'].replace('-', ''),
                                                                 "%Y%m%d")
        form.project_enddate.data = datetime.datetime.strptime(
            db[id]['project_enddate'].replace('-', ''), "%Y%m%d")

        if db[id]['invoice_receipt_date'] != '':
            form.invoice_receipt_date.data = datetime.datetime.strptime(
                db[id]['invoice_receipt_date'].replace('-', ''), "%Y%m%d")
            form.invoice_payout_date.data = datetime.datetime.strptime(
                db[id]['invoice_payout_date'].replace('-', ''), "%Y%m%d")
        ...
        if request.method == 'POST':
            ...
            if form.validate_on_submit_sur() == True:
                ...
                prof_name = request.form.getlist('prof_name')
                prof_college_name = request.form.getlist('prof_college_name')
                doc['list_prof'] = [x for x in zip(
                    prof_name, prof_college_name)]
                db.save(doc)
                print(doc)
        ...
        form.startdate.data = datetime.datetime.strptime(
            db[id]['startdate'].replace('-', ''), "%Y%m%d")
        form.enddate.data = datetime.datetime.strptime(
            db[id]['enddate'].replace('-', ''), "%Y%m%d")
        form.finaledate.data = datetime.datetime.strptime(
            db[id]['finaledate'].replace('-', ''), "%Y%m%d")
        ...
        if request.method == 'POST':
            ...
            if form.validate_on_submit_hackathon() == True:
                ...
                team_member_name1 = request.form.getlist('team_member_name1')
                team_member_college1 = request.form.getlist(
                    'team_member_college1')
                doc['list_team1'] = [x for x in zip(
                    team_member_name1, team_member_college1)]
                doc['team_name2'] = form.team_name2.data
                team_member_name2 = request.form.getlist('team_member_name2')
                team_member_college2 = request.form.getlist(
                    'team_member_college2')
                doc['list_team2'] = [x for x in zip(
                    team_member_name2, team_member_college2)]
                doc['team_name3'] = form.team_name3.data
                team_member_name3 = request.form.getlist('team_member_name3')
                team_member_college3 = request.form.getlist(
                    'team_member_college3')
                doc['list_team3'] = [x for x in zip(
                    team_member_name3, team_member_college3)]
                jury = request.form.getlist('jury_name')
                bu = request.form.getlist('bu')
